...
echo "======================================================================="
echo "|                                                                     |"
echo "|      full-stack-nginx-magento-for-everyone-with-docker-compose      |"
echo "|                      by Erdal ALTIN                                 |"
echo "|                                                                     |"
echo "======================================================================="
sleep 2
...
sed -i 's/db_version/'$db_version'/g' .env
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing Magento and the other services
	docker compose up -d & export pid=$!
	echo "Magento and the other services installing proceeding..."
	echo
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		echo
		echo "Error! could not installed Magento and the other services with docker compose" >&2
		echo
		exit 1
	fi
else
